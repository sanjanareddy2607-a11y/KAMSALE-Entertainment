...
import { filterVenues } from "../data/venues";
...
export function VenueAutocomplete({ value, onChange, error }: VenueAutocompleteProps) {
  const id = useId();
  ...
  const suggestions = filterVenues(value);
      ...
      {open && suggestions.length > 0 && (
        ...
        >
          {suggestions.map((venue, index) => (
            <li key={venue} role="option" aria-selected={index === highlighted}>
              <button
                type="button"
